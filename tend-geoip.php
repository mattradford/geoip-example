...
function tend_country_geo_redirect() {
	if ( is_front_page() ) {
		$countries = array(
    		...
    		$continent = $countries[$current_country]['continent'];

        	switch( $continent ) {
            // Europe
            case "EU";
				if( $_SERVER["REQUEST_URI"] != "/uk/" ) {	
	                wp_redirect( network_site_url() . '/uk/', 302 );
    		        exit();
				}
				break;
            // North America
            case "NA";
				if( $_SERVER["REQUEST_URI"] != "/us/" ) {
	                wp_redirect( network_site_url() . '/us/', 302 );
					exit();
				}
				break;
            // South America
...
add_action( 'template_redirect', 'tend_country_geo_redirect' );